...
5. **← SIGUIENTE** — pantalla admin (crear usuarios, asignar rol, `project_members`) y módulo de inventario. También pendiente: extender el patrón `persistToServer`/migración a `preventivos` cuando ese módulo se conecte a Supabase (hoy solo `att` está migrado).
