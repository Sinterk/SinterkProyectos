...
import { buildClosedPreventivosZip } from '../utils/exportClosedPreventivos'
import type { Preventivo } from '../types'

interface Props {
  /** Levantamientos cerrados ya cargados por Home (con puntos/fotos completos). */
  records: Preventivo[]
...
function sig(records: Preventivo[]): string { return records.map((r) => r.id).sort().join(',') }
...
export function DescargarCerradosPanel({ records, onChanged }: Props) {
  ...
  async function handleDescargar() {
    ...
    try {
      const blob = await buildClosedPreventivosZip(seleccionados, (i, total) => setProgress({ i, total }))
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = `Preventivos_cerrados_${desde}_a_${hasta}.zip`
      a.click()
    ...
    for (const record of seleccionados) {
      try {
        const paths = [
          ...
          ...record.puntos.flatMap((p) => [p.fotoLevantamiento?.storagePath, p.fotoAntes?.storagePath, p.fotoDespues?.storagePath]),
        ]
        await removePhotoObjects(paths)
        await preventivoRepo.save({
          ...record,
          cuadrante: { ...record.cuadrante, fotoPlano: undefined },
          puntos: record.puntos.map((p) => ({ ...p, fotoLevantamiento: undefined, fotoAntes: undefined, fotoDespues: undefined })),
        })
        ok++
      } catch (err) {
    ...
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          Exporta un ZIP con fotos + informes (Entel/levantamiento) de los proyectos cerrados en el
          rango. Para ahorrar espacio en Supabase, luego puedes borrar solo las fotos (el proyecto y
          sus movimientos de material se mantienen intactos).
        </p>
      </div>
      ...
      {showConfirm && (
        <div className="bg-red-950/40 border border-red-700/50 rounded-xl p-3 space-y-2">
          <p className="text-xs text-red-200">
            ...
            proyecto(s) descargados (cuadrante, puntos y movimientos de material NO se tocan).
