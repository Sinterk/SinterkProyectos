...
export function PhotoSlot({ label, emoji, borderColor, previewUrl, editable = true, onCapture, onRemove }: Props) {
  ...
  return (
    <button type="button" onClick={() => inputRef.current?.click()} disabled={loading} {...dropProps}
      className={`w-full h-36 rounded-xl border-2 border-dashed flex flex-col items-center justify-center gap-1 transition-colors disabled:opacity-50 ${isDragging ? 'border-brand-500 bg-brand-500/10' : `${borderColor} bg-slate-800/50 hover:bg-slate-700/50`}`}>
      {loading
