-- Permitir que las salidas dejen el stock en negativo — pedido explícito de
-- Andrés para el período transitivo en que todavía se está cargando el stock
-- real al sistema: hay material que físicamente existe pero que el sistema
-- no conoce, y bloquear el movimiento por eso impide registrar la operación
-- real (el caso concreto: asignar un CMIC a alguien de oficina, rechazado
-- con "Stock físico insuficiente" aunque el material sí estaba).
--
-- Contexto de la inconsistencia que esto corrige: `0008_stock_alertas.sql`
-- había sacado este bloqueo (y quitado el CHECK >= 0 de `stock`), pero
-- `0025_prioridad_instalado.sql` lo volvió a poner dentro de `adjust_stock`
-- con un parámetro `p_permitir_negativo` que por defecto es `false`. Cuando
-- se ajustó `LoteSelect.tsx` para dejar elegir lotes sin stock (v1.26), solo
-- se sacó el candado del CLIENTE — el servidor seguía rechazando. De ahí que
-- "quedamos en que se podía" y en la práctica no se pudiera.
--
-- Ahora todas las salidas pasan `p_permitir_negativo => true`:
--   * bodega de origen de Entrega y de Traspaso entre bodegas
--   * ubicación del técnico en Devuelto y Merma
--   * baja digital de Rebajado
-- `instalado` NO se toca: ya tenía su propia cadena (stock propio -> otro
-- miembro del proyecto -> forzado en negativo con `requiere_revision` y su
-- `eventos_inventario`), que sigue igual.
--
-- El negativo no queda escondido: la pestaña Bodega ya lo marca en rojo con
-- "⚠ Descuadre — revisar", y se reconcilia por Conteo.
--
-- Misma firma, true replace (idéntica a 0047_conteo_asignacion.sql salvo los
-- 5 `adjust_stock` de arriba).
