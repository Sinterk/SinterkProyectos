-- Fix: registrar_movimiento exigía project_id para 'entrega' y 'devuelto',
-- pero eso rompía el caso de "Salida preventiva" (técnico surtiendo su
-- stock personal sin un OTT/incidencia asociado) definido al inicio del
-- diseño de Logística. Ahora project_id es opcional para esos dos tipos —
-- si no hay proyecto, se hace la transferencia de stock igual mostrando en
-- movimientos con project_id=null, pero no se toca proyecto_materiales (no
-- hay proyecto al cual atribuirlo). Solicitud/Instalado/Rebajado siguen
-- exigiendo proyecto (no tienen sentido sin un trabajo concreto detrás).
...
begin
  ...
  if p_tipo_ui = 'entrada' then
    ...
    -- project_id opcional: "Salida preventiva" (surtir al técnico sin OTT asociado).
