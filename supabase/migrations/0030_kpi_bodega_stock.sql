-- El objetivo real de Físico/Digital es comparar el consumo de un periodo
-- contra la cantidad disponible en bodega, para decidir si conviene pedir
-- más — para eso hace falta saber DE QUÉ BODEGA es ese Físico/Digital, sin
-- depender de que haya habido movimiento en el periodo (Origen es de
-- movimientos, no de stock — un material sin actividad ese mes igual tiene
-- stock real, y antes salía sin bodega asociada). Se agrega `bodega_stock`:
-- bodega(s) — dentro de `p_stock_ubicacion_ids` — con Físico o Digital
-- distinto de cero para ese material, siempre que se haya pedido stock.

drop function if exists public.kpi_materiales(text, text, date, date, uuid[], uuid[], uuid[], uuid[]);
...
create or replace function public.kpi_materiales(
  ...
  fisico numeric, digital numeric, bodega_stock text
)
...
begin
  ...
  stock_agg as (
    select s.material_id as sa_material_id,
      sum(s.cantidad_fisico) as sa_fisico, sum(s.cantidad_digital) as sa_digital,
      nullif(array_to_string(array_agg(distinct u.nombre) filter (
        where s.cantidad_fisico <> 0 or s.cantidad_digital <> 0
      ), ', '), '') as sa_bodega
    from public.stock s
  ...
  select
    ...
    sa.sa_fisico, sa.sa_digital, sa.sa_bodega
