-- Fix de 0027_kpi_v2.sql: en plpgsql, cuando una función RETURNS TABLE usa
-- un nombre de columna de salida (ej. `ott`, `material_id`) igual a una
-- columna real de una tabla, cualquier referencia SIN calificar a ese
-- nombre dentro del cuerpo de la función es ambigua para Postgres (no sabe
-- si es el parámetro de salida o la columna de la tabla) — aunque en el
-- momento de escribirla pareciera no haber ambigüedad real. Se detectó recién
-- al correr contra la BD real (no lo pesca una revisión de sintaxis).
-- Fix: calificar explícitamente cada referencia con su alias de tabla/CTE.
...
begin
  ...
  return query
  with clasificado as (
    select p.id as c_id, p.ott as c_ott, p.created_at::date as c_fecha_inicio,
      case
  ...
  select clasificado.c_id, clasificado.c_ott, clasificado.c_estado, clasificado.c_fecha_inicio
  from clasificado
  where clasificado.c_estado is not null
  order by
    case clasificado.c_estado when 'abierto' then 0 when 'pendiente' then 1 when 'cerrado' then 2 end,
    clasificado.c_fecha_inicio asc;
end;
$$;
    ...
    select
      ...
      nullif(array_to_string(array_agg(distinct mov.mv_ubicacion_nombre) filter (
        where mov.mv_es_bodega and mov.mv_fecha::date between p_desde and p_hasta
          and (p_bodega_defecto is null or mov.mv_ubicacion_nombre <> p_bodega_defecto)
      ), ', '), '') as ag_origen_bodega,
