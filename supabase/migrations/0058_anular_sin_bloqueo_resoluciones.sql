-- Se saca el bloqueo por "el evento ya tiene resoluciones" que puse en 0056.
--
-- El mensaje mandaba a "deshacerlas primero desde Conteo/Técnico", pero eso
-- NO EXISTE en la app: `ResolucionRow` solo muestra las resoluciones, no hay
-- forma de revertir una. Era un callejón sin salida — Andrés quedó trabado
-- sin poder borrar una fila de prueba.
--
-- Revisado el diseño, el bloqueo tampoco se justificaba: el efecto real de
-- cada resolución (stock y/o `proyecto_materiales`) quedó registrado en el
-- MOVIMIENTO que esa resolución generó, y ese movimiento sigue en la tabla y
-- se puede anular por separado — con la reversión correcta, porque
-- `anular_movimiento` ya sabe deshacer cada tipo. Las filas de
-- `eventos_inventario_resoluciones` son bookkeeping del evento: si el evento
-- se va (porque se anuló el movimiento que lo generó), se van con él por el
-- cascade que ya existe desde 0039.
--
-- Para deshacer TODO hay que anular también los movimientos que generaron
-- esas resoluciones — se reconocen por su nota ("Ajuste por conteo — …",
-- "Ajuste — devolución no registrada", "Reasignación de consumo al técnico
-- correcto", etc.). El orden no importa: cada uno revierte lo suyo.
--
-- Cuerpo idéntico al de 0057 salvo ese bloque eliminado.
